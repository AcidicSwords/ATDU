...
const CATEGORIES = ["Habit", "Contested", "Intended"];
const CAT_COLORS = { Habit: COLOR.inkL, Contested: COLOR.gold, "Intended": "#8A6565" };
...
.today-card { padding: 20px; border-radius: 8px; background: ${COLOR.white}; border: 1px solid ${COLOR.border}; transition: all 0.15s ease; overflow: hidden; word-break: break-word; }
...
function categoryDefault(w) { return w.category === "Intended" ? OUT.MINUS : OUT.PLUS; }

function catDesc(cat) {
  if (cat === "Habit") return "This has been going one way. You think it could go differently.";
  if (cat === "Contested") return "This has gone both ways. There's always a reason for either.";
  if (cat === "Intended") return "You would. You could. You haven't.";
  return "";
}
...
function SituationForm(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div>
        <label style={S.label}>Wager</label>
        <input style={S.input} value={d.name} onChange={function(e) { set("name", e.target.value); }} placeholder="Something that keeps coming up" />
      </div>
      ...
      <div>
        <label style={{ ...S.label, marginBottom: 8 }}>Two ways it goes</label>
        <div className="mobile-stack" style={{ display: "flex", gap: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <input style={{ ...S.input, borderLeft: "3px solid " + COLOR.sideA + "60" }} value={d.plus}
              onChange={function(e) { set("plus", e.target.value); }} placeholder="What has been happening" />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <input style={{ ...S.input, borderLeft: "3px solid " + COLOR.sideB + "60" }} value={d.minus}
              onChange={function(e) { set("minus", e.target.value); }} placeholder="What could happen" />
          </div>
        </div>
      </div>
  ...
  var finals = useMemo(function() {
    ...
    return SIM_PROFILES.map(function(p, i) {
      ...
      entries.forEach(function(e, j) {
        if (e.mode !== "U") return;
        var prevU = null, nextU = null;
        for (var pi = j - 1; pi >= 0; pi--) { if (entries[pi].mode === "U") { prevU = entries[pi].outcome; break; } }
        for (var ni = j + 1; ni < entries.length; ni++) { if (entries[ni].mode === "U") { nextU = entries[ni].outcome; break; } }
        var bright = prevU === e.outcome && (nextU === null || nextU === e.outcome);
        if (bright && e.outcome === OUT.PLUS) { cP++; cM = 0; }
        else if (bright && e.outcome === OUT.MINUS) { cM++; cP = 0; }
        else { cP = 0; cM = 0; }
        var b = cP > cM ? cP : cM;
        if (b > maxR) maxR = b;
      });
    ...
    <div style={{ maxWidth: 780, margin: "0 auto" }}>
      <div style={{ ...S.card, marginTop: 24 }}>
        <div style={{ display: "grid", gap: 10 }}>
          {["You place a wager. What has been happening, and what could.", "Each day a coin is flipped.", "Sometimes you choose. Sometimes it is assigned based on what happened before.", "The ledger records what happened. It does not explain why."].map(function(t, i) {
      ...
      <div style={{ marginTop: 28 }}>
        ...
        {results && finals && convergence && (<>
          <div style={{ ...S.card, padding: 20 }}>
            ...
            <p style={{ fontSize: 12, color: COLOR.inkL, margin: "0 0 12px" }}>Each row is one year. Bright = chosen streak. Dim = chosen. Faded = assigned.</p>
            {SIM_PROFILES.map(function(p, i) {
              var data = results[p.key];
              return (
                <div key={p.key} style={{ marginBottom: 2, display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                    {data.map(function(e, j) {
                      var prevU = null, nextU = null;
                      for (var pi = j - 1; pi >= 0; pi--) { if (data[pi].mode === "U") { prevU = data[pi].outcome; break; } }
                      for (var ni = j + 1; ni < data.length; ni++) { if (data[ni].mode === "U") { nextU = data[ni].outcome; break; } }
                      var bright = e.mode === "U" && prevU === e.outcome && (nextU === null || nextU === e.outcome);
                      var op = e.mode === "C" ? 0.25 : bright ? 1 : 0.45;
                      return <div key={j} style={{ width: 6, height: 6, borderRadius: 1, background: e.outcome === OUT.PLUS ? COLOR.sideA : COLOR.sideB, opacity: op }} />;
                    })}
                  </div>
                </div>
              );
            })}
            <div style={{ display: "flex", gap: 12, justifyContent: "center", fontSize: 11, color: COLOR.inkL, marginTop: 8 }}>
              <span><span style={{ display: "inline-block", width: 6, height: 6, background: COLOR.sideA, borderRadius: 1, marginRight: 3, verticalAlign: "middle" }} />Side A</span>
              <span><span style={{ display: "inline-block", width: 6, height: 6, background: COLOR.sideB, borderRadius: 1, marginRight: 3, verticalAlign: "middle" }} />Side B</span>
            </div>
          </div>
                ...
                return (
                  <div key={f.key} style={{ padding: "12px 16px", borderRadius: 6, borderLeft: "3px solid " + f.color, background: f.color + "08" }}>
                    ...
                    <div style={{ display: "flex", gap: 16, fontSize: 13, color: COLOR.inkL, flexWrap: "wrap" }}>
                      ...
                      <span>Longest chosen streak: <strong style={{ color: COLOR.ink }}>{f.longestRun}</strong></span>
                    </div>
                  </div>
                );
              })}
      ...
      <div style={{ marginTop: 28 }}>
        <h3 style={{ ...S.h3, margin: "0 0 12px" }}>{active.length ? "Wagers" : "Place a wager"}</h3>
        {active.length > 0 && (
        ...
        {adding ? (
          ...
          <button onClick={function() { setAdding(true); }} style={{ ...S.btn, background: "transparent", border: "1px solid " + COLOR.border, color: COLOR.inkL }}>
            {active.length ? "Add another" : "Place your first wager"}
          </button>
        )}
        {active.length > 0 && (
  ...
  if (!situations || !active.length) {
    return (
      <div style={{ maxWidth: 780, margin: "0 auto", textAlign: "center", padding: "60px 20px" }}>
        <p style={{ ...S.muted, marginBottom: 20 }}>No wagers placed yet.</p>
...
function RecordTab(props) {
  ...
  var _selW = useState(null), selectedWager = _selW[0], setSelectedWager = _selW[1];
  var active = situations ? situations.filter(function(w) { return !w.removed; }) : [];
  ...
  var isBrightU = useCallback(function(idx, code) {
    ...
    for (var i = idx - 1; i >= 0; i--) {
      var p = ledger[i] ? ledger[i][code] : null;
      if (p && p.mode === "U" && normalizeOutcome(p.outcome)) { prevU = normalizeOutcome(p.outcome); break; }
    }
    for (var j = idx + 1; j < ledger.length; j++) {
      var n = ledger[j] ? ledger[j][code] : null;
      if (n && n.mode === "U" && normalizeOutcome(n.outcome)) { nextU = normalizeOutcome(n.outcome); break; }
    }
    if (prevU !== o) return false;
    if (nextU !== null && nextU !== o) return false;
    return true;
  }, [ledger]);
  ...
  var analysis = useMemo(function() {
    ...
    return active.map(function(w) {
      ...
      entries.forEach(function(e, i) {
        if (e.mode !== "U") { e.bright = false; return; }
        var prevU = null, nextU = null;
        for (var p = i - 1; p >= 0; p--) { if (entries[p].mode === "U") { prevU = entries[p].outcome; break; } }
        for (var n = i + 1; n < entries.length; n++) { if (entries[n].mode === "U") { nextU = entries[n].outcome; break; } }
        if (prevU !== e.outcome) { e.bright = false; return; }
        if (nextU !== null && nextU !== e.outcome) { e.bright = false; return; }
        e.bright = true;
      });
      var mR = 0, cR = 0;
      entries.forEach(function(e) { if (e.bright && isL(e.outcome)) { cR++; if (cR > mR) mR = cR; } else { cR = 0; } });
      var mRO = 0, cRO = 0;
      entries.forEach(function(e) { if (e.bright && !isL(e.outcome)) { cRO++; if (cRO > mRO) mRO = cRO; } else { cRO = 0; } });
      var cum = 0;
      var rd = entries.map(function(e, i) {
        if (isL(e.outcome)) cum++;
        return { day: e.day, ratio: cum / (i + 1), outcome: e.outcome, mode: e.mode, bright: e.bright, y: e.outcome === OUT.PLUS ? 1 : 0 };
      });
  ...
  if (!ledger.length) {
    return <div style={{ maxWidth: 780, margin: "0 auto", textAlign: "center", padding: "60px 20px" }}><p style={S.muted}>No days in the ledger yet.</p></div>;
  }
    ...
    <div style={{ maxWidth: 780, margin: "0 auto" }}>
      <div style={{ marginTop: 24 }}>
        ...
        <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
                        ...
                        {recordSits.map(function(w) {
                          ...
                          return (
                            <td key={w.code} style={{ textAlign: "center", padding: "8px 6px" }}>
                              <span style={{ display: "inline-block", width: 14, height: 14, borderRadius: 3, background: outcomeColor(o), opacity: op }}
                                title={actionText(w, o) + " (" + (e.mode === "C" ? "assigned" : bright ? "chosen, in streak" : "chosen") + ")"} />
                            </td>
                          );
                        })}
                      </tr>
          ...
          <div style={{ padding: "8px 16px 12px", display: "flex", gap: 16, justifyContent: "center", fontSize: 12, flexWrap: "wrap", borderTop: "1px solid " + COLOR.borderL }}>
            <span><span style={{ display: "inline-block", width: 8, height: 8, background: COLOR.sideA, borderRadius: 2, marginRight: 4, verticalAlign: "middle" }} /><span style={{ color: COLOR.inkL }}>Side A</span></span>
            <span><span style={{ display: "inline-block", width: 8, height: 8, background: COLOR.sideB, borderRadius: 2, marginRight: 4, verticalAlign: "middle" }} /><span style={{ color: COLOR.inkL }}>Side B</span></span>
            <span style={{ color: COLOR.inkL }}>bright = chosen streak · dim = chosen · faded = assigned</span>
          </div>
        </div>
      </div>
      ...
      {analysis.length > 0 && (
        <div style={{ marginTop: 28 }}>
          {analysis.length > 1 && (
            <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
              {analysis.map(function(a) {
                var sel = selectedWager === a.w.code;
                return (
                  <button key={a.w.code} onClick={function() { setSelectedWager(a.w.code); }}
                    style={{ ...S.btn, padding: "6px 14px", fontSize: 13, minHeight: 36,
                      background: sel ? COLOR.ink : "transparent",
                      color: sel ? COLOR.paper : COLOR.inkL,
                      border: "1px solid " + (sel ? COLOR.ink : COLOR.border),
                    }}>
                    {a.w.code}
                  </button>
                );
          ...
          {(function() {
            var a = analysis.find(function(x) { return x.w.code === selectedWager; }) || analysis[0];
            if (!a) return null;
            var w = a.w;
            var seq = a.entries.length > 180 ? a.entries.slice(-180) : a.entries;
            return (
              <div style={{ ...S.card, padding: 0, overflow: "hidden", marginBottom: 0 }}>
                <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid " + COLOR.borderL }}>
                  ...
                  <p style={{ fontSize: 13, color: COLOR.inkL, margin: 0 }}>
                    When you choose: <strong style={{ color: a.leanColor }}>{a.leanLabel}</strong>{" "}{a.uLeanPct}% of the time
                  </p>
                </div>

                <div style={{ padding: "12px 20px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(90px, 1fr))", gap: "8px 16px", fontSize: 13, color: COLOR.inkL, borderBottom: "1px solid " + COLOR.borderL }}>
                  <div><div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.04em", marginBottom: 2, textTransform: "uppercase" }}>Days</div><strong style={{ color: COLOR.ink, fontSize: 16 }}>{a.totalDays}</strong></div>
                  <div><div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.04em", marginBottom: 2, textTransform: "uppercase" }}>Overall</div><strong style={{ color: a.leanColor, fontSize: 16 }}>{a.overallPct}%</strong></div>
                  <div><div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.04em", marginBottom: 2, textTransform: "uppercase" }}>Assigned</div><strong style={{ color: COLOR.ink, fontSize: 16 }}>{a.cDays}</strong><span style={{ fontSize: 11 }}>/{a.totalDays}</span></div>
                  <div><div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.04em", marginBottom: 2, textTransform: "uppercase" }}>Streak</div>
                    <span style={{ color: a.leanColor }}><strong style={{ fontSize: 16 }}>{a.maxRunLean}</strong></span>
                    <span style={{ margin: "0 4px", color: COLOR.borderL }}>/</span>
                    <span style={{ color: a.otherColor }}><strong style={{ fontSize: 16 }}>{a.maxRunOther}</strong></span>
                  </div>
                </div>

                <div style={{ padding: "12px 20px", borderBottom: "1px solid " + COLOR.borderL }}>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
                    {seq.map(function(e, i) {
                      var op = e.mode === "C" ? 0.25 : e.bright ? 1 : 0.45;
                      return <div key={i} title={"Day " + e.day + ": " + actionText(w, e.outcome) + " (" + (e.mode === "C" ? "assigned" : "chosen") + ")"}
                        style={{ width: 7, height: 7, borderRadius: 1, background: outcomeColor(e.outcome), opacity: op }} />;
                    })}
                  </div>
                </div>
                ...
                {a.runningData.length > 2 && (
                  <div style={{ padding: "12px 20px 16px" }}>
                    <ResponsiveContainer width="100%" height={200}>
                      <LineChart data={a.runningData} margin={{ top: 10, right: 8, bottom: 0, left: -10 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke={COLOR.borderL} />
                        <XAxis dataKey="day" tick={{ fontSize: 10, fill: COLOR.inkL }} />
                        <YAxis domain={[0, 1]} ticks={[0, 0.5, 1]} tick={{ fontSize: 10, fill: COLOR.inkL }} tickFormatter={function(v) { return v === 1 ? "A" : v === 0 ? "B" : ""; }} />
                        <ReferenceLine y={0.5} stroke={COLOR.gold} strokeDasharray="4 4" strokeWidth={1.5} />
                        <Tooltip contentStyle={{ fontSize: 11, fontFamily: FONT.sans }} formatter={function(v, name) { return name === "ratio" ? (v * 100).toFixed(1) + "%" : v === 1 ? w.plus : w.minus; }} />
                        <Line type="monotone" dataKey="ratio" stroke={COLOR.inkL} strokeWidth={1.5} dot={false} opacity={0.5} />
                        <Line dataKey="y" stroke="transparent" strokeWidth={0} isAnimationActive={false}
                          dot={function(dotProps) {
                            if (!dotProps || dotProps.index == null) return null;
                            var d = a.runningData[dotProps.index];
                            if (!d) return null;
                            var op = d.mode === "C" ? 0.25 : d.bright ? 1 : 0.45;
                            return <circle key={dotProps.index} cx={dotProps.cx} cy={dotProps.cy} r={3} fill={outcomeColor(d.outcome)} fillOpacity={op} stroke="none" />;
                          }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                ...
                <div style={{ padding: "6px 20px 10px", display: "flex", gap: 16, justifyContent: "center", fontSize: 11, color: COLOR.inkL, flexWrap: "wrap", borderTop: "1px solid " + COLOR.borderL }}>
                  <span><span style={{ display: "inline-block", width: 6, height: 6, background: COLOR.sideA, borderRadius: 1, marginRight: 3, verticalAlign: "middle" }} />Side A</span>
                  <span><span style={{ display: "inline-block", width: 6, height: 6, background: COLOR.sideB, borderRadius: 1, marginRight: 3, verticalAlign: "middle" }} />Side B</span>
                  <span>bright = chosen streak · dim = chosen · faded = assigned</span>
                  <span><span style={{ display: "inline-block", width: 8, height: 1.5, background: COLOR.gold, marginRight: 3, verticalAlign: "middle" }} />center</span>
                </div>
              </div>
            );
          })()}
